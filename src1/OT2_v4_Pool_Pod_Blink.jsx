...
// ============================================================================
// OT² v4 — Pool · Pod · Blink Edition  (Changelog applied)
// ============================================================================
...
const STORAGE_KEYS = {
  ...
  HIDE_INTRO: 'ot2_hide_intro_video',
};
...
const Icons = {
  ...
  Video: ({ className = "w-4 h-4" }) => (<svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>),
  BookOpen: ({ className = "w-4 h-4" }) => (<svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>),
  MessageSquare: ({ className = "w-4 h-4" }) => (<svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>),
  Info: ({ className = "w-4 h-4" }) => (<svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>),
  Bold: ({ className = "w-4 h-4" }) => (<svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 4h8a4 4 0 0 1 4 4 4 4 0 0 1-4 4H6z"/><path d="M6 12h9a4 4 0 0 1 4 4 4 4 0 0 1-4 4H6z"/></svg>),
...
// ============================================================================
function VideoPopup({ onClose, onHideForever }) {
  const [hideChecked, setHideChecked] = useState(false);
  const handleClose = () => {
    if (hideChecked) onHideForever();
    onClose();
  };
  return (
    <div style={{
      ...
      <div style={{
        backgroundColor: 'white', borderRadius: 16, width: '100%', maxWidth: 680,
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #E4E4E7' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Icons.Video className="w-5 h-5" style={{ color: '#FF6B6B' }} />
            <span style={{ fontSize: 16, fontWeight: 700, color: '#18181B' }}>Getting Started with OT²</span>
          </div>
          <button onClick={handleClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#71717A', display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: '#52525B' }}>
            <input
              type="checkbox"
              checked={hideChecked}
              onChange={e => setHideChecked(e.target.checked)}
              style={{ accentColor: '#FF6B6B', width: 16, height: 16 }}
            />
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <a
              ...
              style={{ fontSize: 13, color: '#4299E1', textDecoration: 'none' }}
            >
              Watch on YouTube ↗
            </a>
            <button onClick={handleClose} style={{ padding: '8px 20px', backgroundColor: '#FF6B6B', color: 'white', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
              Get Started
            </button>
          </div>
        </div>
...
function NavDropdown({ label, items }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: '#52525B', fontWeight: 500, padding: '4px 6px', borderRadius: 6 }}
      >
        {label}
      ...
      {open && (
        <div style={{
          ...
          backgroundColor: 'white', border: '1px solid #E4E4E7', borderRadius: 10,
          boxShadow: '0 8px 24px rgba(0,0,0,0.1)', zIndex: 200, padding: 6
        }}>
          {items.map((item, i) => (
            <a
              key={i}
              ...
              style={{
                display: 'block', padding: '9px 12px', borderRadius: 7, fontSize: 14,
                color: '#18181B', textDecoration: 'none', cursor: 'pointer',
...
// ============================================================================
// RICH TEXT INPUT (multiline + optional formatting toolbar)
// ============================================================================
function RichTextInput({ value, onChange, onSubmit, placeholder }) {
  const [richMode, setRichMode] = useState(false);
  const textareaRef = useRef(null);

  const handleKeyDown = (e) => {
    if (e.key === 'Enter' && !e.shiftKey && !richMode) { e.preventDefault(); onSubmit(); }
  };
  ...
  const applyFormat = (tag) => {
    ...
    let newText = value;
    if (tag === 'bold') newText = value.slice(0, start) + `**${selected}**` + value.slice(end);
    if (tag === 'italic') newText = value.slice(0, start) + `_${selected}_` + value.slice(end);
    if (tag === 'bullet') newText = value.slice(0, start) + `\n• ${selected}` + value.slice(end);
    onChange(newText);
  };
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
        <button
          ...
          title={richMode ? 'Switch to simple mode' : 'Enable rich text'}
          style={{
            fontSize: 11, padding: '3px 8px', border: '1px solid #D4D4D8', borderRadius: 5,
            cursor: 'pointer', backgroundColor: richMode ? '#FF6B6B' : 'white',
            color: richMode ? 'white' : '#71717A', fontWeight: 600
          }}
        >
          {richMode ? '✦ Rich' : '✦ Rich'}
        </button>
        {richMode && (
          <>
            <button type="button" onClick={() => applyFormat('bold')} title="Bold" style={{ ...styles.formatBtn }}><Icons.Bold className="w-3.5 h-3.5" /></button>
            <button type="button" onClick={() => applyFormat('italic')} title="Italic" style={{ ...styles.formatBtn }}><Icons.Italic className="w-3.5 h-3.5" /></button>
            <button type="button" onClick={() => applyFormat('bullet')} title="Bullet" style={{ ...styles.formatBtn }}><Icons.List className="w-3.5 h-3.5" /></button>
          </>
        )}
        <span style={{ fontSize: 11, color: '#A1A1AA', marginLeft: 'auto' }}>{richMode ? 'Shift+Enter for new line' : 'Enter to add · Shift+Enter new line'}</span>
      </div>
      <textarea
        ref={textareaRef}
        ...
        rows={richMode ? 4 : 2}
        style={{
          width: '100%', padding: '10px 14px', border: '1px solid #D4D4D8', borderRadius: 8,
          fontSize: 15, outline: 'none', backgroundColor: 'white', resize: 'vertical',
          fontFamily: 'inherit', boxSizing: 'border-box', lineHeight: 1.5,
          minHeight: 44, transition: 'min-height .2s'
        }}
      />
    </div>
  );
...
export default function OT2App() {
  ...
  const [authForm, setAuthForm] = useState({ email: '', profileName: '', password: '', confirmPassword: '', otpCode: '', newPassword: '' });
  ...
  const [focusTaskType, setFocusTaskType] = useState('blink');
  ...
  useEffect(() => {
    try {
      ...
      // Show intro video unless user hid it
      const hideIntro = localStorage.getItem(STORAGE_KEYS.HIDE_INTRO);
      if (!hideIntro) setShowVideoPopup(true);
  ...
  // FIX-2: completeTask and uncompleteTask
  ...
  const startFocus = (taskId) => {
    ...
    setFocusTypeConfirmed(!!(task?.reflection));
  ...
  const skipTask = () => {
    ...
    if (nextTask) { startFocus(nextTask.id); } else { setFocusedTaskId(null); setGuestMode('work'); }
  };
  ...
  const setPodLog = (podId, taskId, date, updates) => {
    ...
    // FIX-9: Do not allow future dates to be marked complete
  ...
  const handleLogin = () => {
    ...
    const found = TEST_USERS.find(u => u.email === authForm.email && u.password === authForm.password);
    if (!found) { setAuthError('Invalid credentials'); return; }
    setAuthLoading(true);
    setTimeout(() => {
      setAuthLoading(false);
      const u = { email: found.email, profileName: found.profileName };
      setUser(u); setIsAuthenticated(true);
      localStorage.setItem(STORAGE_KEYS.USER, JSON.stringify(u));
      setPage('home'); setAuthForm({ email: '', profileName: '', password: '', confirmPassword: '', otpCode: '', newPassword: '' });
    }, 800);
  };
  const handleLogout = () => { setUser(null); setIsAuthenticated(false); localStorage.removeItem(STORAGE_KEYS.USER); };

  const handleHideIntroForever = () => { localStorage.setItem(STORAGE_KEYS.HIDE_INTRO, '1'); };
    ...
    <div style={styles.app}>
      ...
      {showVideoPopup && (
        <VideoPopup
          ...
          onHideForever={handleHideIntroForever}
        />
      )}
        ...
        <div style={styles.headerContent}>
          ...
          <nav style={styles.headerNav}>
            ...
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: '#52525B', fontWeight: 500, padding: '4px 6px', borderRadius: 6 }}
            >
              <Icons.Info className="w-3.5 h-3.5" /> About
            </button>

            {/* Resources */}
            <NavDropdown label="Resources" items={[
              { label: '📝 Blog', href: '#', icon: null },
              { label: '🎬 Videos / Reels', href: '#', icon: null },
              { label: '📚 Books & References', href: '#', icon: null },
            ]} />

            {/* Contact */}
            <NavDropdown label="Contact" items={[
          ...
          <div style={styles.headerRight}>
            {isAuthenticated ? (
              <>
                <span style={styles.greeting}>Hi, {user?.profileName}</span>
...
// ============================================================================
// FREEDOM MODE — FIX-1 (no completed tasks), FIX-3 (multiline input), FIX-2 (clickable)
// ============================================================================
function FreedomMode({ newTask, setNewTask, onAddTask, pendingTasks, allPendingCount, onDeleteTask, onStartFocus, onCompleteTask }) {
  const qualifiedCount = (allPendingCount || 0) - pendingTasks.length;
  return (
    <div>
      {/* FIX-3: Multiline rich text input */}
      <div style={styles.inputRow}>
        <RichTextInput
          value={newTask}
          onChange={setNewTask}
          onSubmit={onAddTask}
          placeholder="What's on your mind? (Enter to add · Shift+Enter new line)"
        />
        <button style={{ ...styles.iconBtn, alignSelf: 'flex-start', marginTop: 28 }} onClick={onAddTask} title="Add task">
          <Icons.Plus />
        </button>
      </div>
      ...
      {/* FIX-6: Minimal task area — no forced minHeight */}
      <div style={styles.taskArea}>
        {pendingTasks.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '24px 20px', color: '#A1A1AA', fontSize: 14 }}>
            <Icons.Sparkles className="w-6 h-6" style={{ margin: '0 auto 8px', color: '#D4D4D8' }} />
            Add your first task to get started
          </div>
        ) : (
          <div style={styles.taskList}>
            {/* FIX-2: clicking task opens focus mode (always) */}
            {pendingTasks.map(task => (
              <div key={task.id} style={{ ...styles.taskItem, cursor: 'pointer' }}
                onClick={() => onStartFocus(task.id)}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, flex: 1 }}>
                  {/* Completion circle */}
                  <button
                    style={{ ...styles.checkCircle, flexShrink: 0, marginTop: 2 }}
                    onClick={e => { e.stopPropagation(); onCompleteTask(task.id); }}
                    title="Mark complete"
                  />
                  <div style={{ flex: 1 }}>
                    {task.type && task.type !== 'blink' && (
                      <span style={{ ...styles.typeBadge, backgroundColor: task.type === 'pool' ? '#6366F115' : '#0EA5E915', color: task.type === 'pool' ? '#6366F1' : '#0EA5E9', display: 'inline-block', marginBottom: 2 }}>
        ...
        {/* FIX-1: completed tasks NOT shown in Freedom mode */}
      </div>
    </div>
  );
}
...
function PoolComboBox({ pools, onSelect, onCreatePool, selectedPoolId }) {
  ...
  const handleCreate = () => {
    ...
    onSelect(pool.id); setSearch(''); setOpen(false);
  };
  useEffect(() => {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <div style={styles.comboInput} onClick={() => setOpen(true)}>
        {selected ? <span style={{ color: '#18181B', fontWeight: 500 }}>⊕ {selected.name}</span> : <span style={{ color: '#A1A1AA' }}>Search or create a Pool…</span>}
        <Icons.ChevronDown className="w-4 h-4" />
      </div>
      {open && (
        <div style={styles.comboDropdown}>
          <input autoFocus type="text" placeholder="Search or type new pool name…" value={search} onChange={e => setSearch(e.target.value)} style={{ ...styles.input, marginBottom: 8 }} onKeyDown={e => { if (e.key === 'Enter') handleCreate(); }} />
          ...
          {search.trim() && !filtered.some(p => p.name.toLowerCase() === search.toLowerCase()) && (
            <button style={{ ...styles.primaryBtn, width: '100%', marginTop: 8, justifyContent: 'center', backgroundColor: '#6366F1' }} onClick={handleCreate}>
              <Icons.Plus className="w-4 h-4" /> Create Pool "{search}"
            </button>
          )}
        </div>
      )}
...
function PoolRelationshipPanel({ currentTaskId, poolId, pools, tasks, relationships, onAddRelationship, onRemoveRelationship }) {
  const [relType, setRelType] = useState('precede');
  const [selectedTaskIds, setSelectedTaskIds] = useState([]);
  const pool = pools.find(p => p.id === poolId);
  const poolTasks = tasks.filter(t => (t.poolIds || []).includes(poolId) && t.id !== currentTaskId);
  ...
  return (
    <div style={styles.relPanel}>
      <p style={styles.relTitle}>Link tasks in "{pool.name}" pool</p>
      {poolTasks.length === 0 ? <p style={{ fontSize: 13, color: '#A1A1AA' }}>No other tasks in this pool yet.</p> : (
        <>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
            {RELATIONSHIP_TYPES.map(rt => (
              <button key={rt.key} onClick={() => setRelType(rt.key)} style={{ ...styles.relTypeBtn, ...(relType === rt.key ? { backgroundColor: rt.color, color: 'white', borderColor: rt.color } : {}) }}>
                {rt.icon} {rt.label}
              </button>
            ))}
          </div>
...
// ============================================================================
// POD PICKER — FIX-8: default tracker field type is "text" (free input)
// ============================================================================
function PodPicker({ pods, selectedPodId, onSelect, onCreatePod }) {
  ...
  const [form, setForm] = useState({
    ...
    frequency: 3,
    everyNDays: 2,
    // FIX-8: default tracker field type to 'text' (free input)
    trackerFields: [{ id: uid(), name: '', type: 'text' }],
  });
  ...
  const handleCreate = () => {
    ...
    const pod = podType === 'annual_dates'
      ...
      : onCreatePod({
          ...base,
          recurrence: { type: form.recurrenceType, weekDays: form.weekDays, frequency: form.frequency, everyNDays: form.everyNDays },
  ...
  // FIX-8: new field defaults to 'text'
  const addField = () => { if (form.trackerFields.length >= 5) return; setForm(p => ({ ...p, trackerFields: [...p.trackerFields, { id: uid(), name: '', type: 'text' }] })); };
              ...
              <div key={p.id} onClick={() => onSelect(p.id)} style={{ ...styles.podPickerItem, ...(p.id === selectedPodId ? { borderColor: '#0EA5E9', backgroundColor: '#0EA5E910' } : {}) }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    {/* FIX-7: use text icon to ensure mobile rendering */}
                    <span style={{ color: '#0EA5E9', fontSize: 14, flexShrink: 0 }}>↻</span>
                    <span style={{ fontWeight: 600, fontSize: 14 }}>{p.name}</span>
                  </div>
                  <div style={{ fontSize: 12, color: '#71717A', marginTop: 2, paddingLeft: 20 }}>{podSummaryLine(p)}</div>
                </div>
                {p.id === selectedPodId && <span style={{ color: '#0EA5E9', fontSize: 16 }}>✓</span>}
              </div>
            ))}
            {pods.length === 0 && <p style={{ color: '#A1A1AA', fontSize: 13 }}>No Pods yet — create one below.</p>}
          </div>
          {/* FIX-7: Use text label alongside icon for better mobile rendering */}
          <button style={{ ...styles.primaryBtn, backgroundColor: '#0EA5E9', flexShrink: 0 }} onClick={() => setCreating(true)}>
            <span style={{ fontSize: 16, lineHeight: 1 }}>+</span> Create new Pod
          </button>
        </>
      ) : (
            ...
            <>
              ...
              {form.trackerFields.map((f, i) => (
                <div key={f.id} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
                  ...
                  <select value={f.type} onChange={e => updateField(i, 'type', e.target.value)} style={{ ...styles.input, width: 120 }}>
                    {/* FIX-8: Free input is first/default */}
...
function DoneCard({ wizardAnswers, questions, setQuestions, setWizardStep, setWizardAnswers, onFinish, task }) {
  ...
  const handleMoreQuestions = async () => {
    setLoadingMore(true);
    try {
      const answeredContext = Object.entries(wizardAnswers).filter(([, v]) => v).map(([k, v]) => `${k}: ${v}`).join('\n');
      ...
      setWizardStep(questions.length);
    } finally { setLoadingMore(false); setExtended(true); }
  };
    ...
    <div style={styles.wizardCard}>
      ...
      {!extended && (
        <div style={{ textAlign: 'center', marginBottom: 14 }}>
          <button style={{ background: 'none', border: 'none', color: '#4299E1', fontSize: 13, cursor: loadingMore ? 'default' : 'pointer', textDecoration: 'underline', display: 'inline-flex', alignItems: 'center', gap: 5, opacity: loadingMore ? 0.6 : 1 }} onClick={handleMoreQuestions} disabled={loadingMore}>
...
// FOCUS MODE
// ============================================================================
function FocusMode({ task, pendingTasks, wizardStep, setWizardStep, wizardAnswers, setWizardAnswers, focusTaskType, setFocusTaskType, focusPoolId, setFocusPoolId, focusPodId, setFocusPodId, focusRelationships, setFocusRelationships, focusPodTaskDate, setFocusPodTaskDate, focusTypeConfirmed, onConfirmType, pools, pods, onCreatePool, onCreatePod, tasks, onFinish, onSkipTask }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const init = {};
        result.questions.forEach(q => { init[q.key] = wizardAnswers[q.key] || task.reflection?.[q.key] || ''; });
        setWizardAnswers(prev => ({ ...prev, ...init }));
  ...
  return (
    <div style={styles.focusMode}>
      <div style={styles.progressDots}>
        {pendingTasks.map(t => (<div key={t.id} style={{ ...styles.dot, ...(t.id === task.id ? styles.dotActive : {}) }} />))}
      </div>
      ...
      {!focusTypeConfirmed ? (
        <div style={styles.wizardCard}>
          ...
          {focusTaskType === 'pool' && (
            <div style={{ marginBottom: 16 }}>
              ...
              {focusPoolId && (
                <PoolRelationshipPanel currentTaskId={task.id} poolId={focusPoolId} pools={pools} tasks={tasks} relationships={focusRelationships} onAddRelationship={addRelationship} onRemoveRelationship={removeRelationship} />
              )}
            </div>
          )}
              ...
              {focusPodId && (() => {
                ...
                return (
                  <div style={{ marginTop: 14, backgroundColor: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: 10, padding: 14 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#0369A1', marginBottom: 8 }}>📅 When in the year does <em>"{task.content}"</em> happen?</p>
                    <p style={{ fontSize: 12, color: '#71717A', marginBottom: 10 }}>Pick the day and month only — the year is ignored, this recurs annually.</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <input type="date" value={focusPodTaskDate ? `${new Date().getFullYear()}-${focusPodTaskDate}` : `${new Date().toISOString().slice(0, 10)}`} onChange={e => { const val = e.target.value; if (val) setFocusPodTaskDate(val.slice(5)); }} style={{ ...styles.input, flex: 1 }} />
                      {focusPodTaskDate && <span style={{ fontSize: 13, color: '#0369A1', fontWeight: 600 }}>📌 {new Date(`${new Date().getFullYear()}-${focusPodTaskDate}`).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>}
                    </div>
                  </div>
                );
              })()}
        ...
        <DoneCard wizardAnswers={wizardAnswers} questions={questions} setQuestions={setQuestions} setWizardStep={setWizardStep} setWizardAnswers={setWizardAnswers} onFinish={onFinish} task={task} />
      )}

      {onSkipTask && <div style={styles.skipSection}><button style={styles.linkBtn} onClick={onSkipTask}>Skip this task for now</button></div>}
    </div>
  );
}

// ============================================================================
// REFLECTION SUMMARY CHIP — FIX-4 helper
// ============================================================================
    ...
    <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 3 }}>
      {entries.slice(0, 3).map(([key, value]) => (
        <div key={key} style={{ display: 'flex', gap: 6, fontSize: 12 }}>
          <span style={{ fontWeight: 600, color: '#52525B', textTransform: 'capitalize', whiteSpace: 'nowrap', minWidth: 70 }}>{key}:</span>
          <span style={{ color: '#71717A', flex: 1, lineHeight: 1.4 }}>{String(value)}</span>
        </div>
      ))}
    </div>
  );
...
function WorkMode({
  ...
  // FIX-2: FocusBtn — in work mode, click opens focus for revising answers
  const FocusBtn = ({ taskId }) => (
    <button
      title="Open in Focus Mode (revise answers)"
  ...
  // FIX-2 & FIX-4: TaskCard shows reflection answers and is clickable to complete/uncomplete
    ...
    return (
      <div style={styles.taskCard}>
        <div style={styles.taskCardHeader}>
          {/* FIX-2: clicking checkbox completes task */}
          <button
            style={{ ...styles.checkCircle, flexShrink: 0 }}
            onClick={() => onCompleteTask(task.id)}
            title="Mark complete"
          />
          <span style={{ ...styles.taskCardTitle, cursor: 'pointer' }} onClick={() => onStartFocus(task.id)}>{task.content}</span>
          {task.type && task.type !== 'blink' && (
        ...
        {/* FIX-4: Show reflection answers in card */}
        {!compact && task.reflection && (
          <ReflectionSummary reflection={task.reflection} />
        )}
        {compact && task.reflection && (
          <div
            style={{ fontSize: 11, color: '#A1A1AA', marginTop: 3, cursor: 'pointer' }}
            title={Object.entries(task.reflection || {}).filter(([,v])=>v).map(([k,v])=>`${k}: ${v}`).join('\n')}
          >
            {Object.values(task.reflection || {}).filter(v => v).length > 0 && '💡 Has reflection notes'}
          </div>
        )}
  ...
  // FIX-4: TableView shows reflection answers
  const TableView = () => (
      ...
      <table style={styles.table}>
        <thead>
          <tr>
            ...
            <th style={styles.th}>Outcome / Why</th>
        ...
        <tbody>
          {pendingTasks.map(task => {
            ...
            const running = isTimerRunning(task.id);
            const paused = isTimerPaused(task.id);
            const outcome = task.reflection?.outcome || '';
            const motivation = task.reflection?.motivation || '';
            return (
              <tr key={task.id} style={styles.tr}>
                <td style={styles.td}>
                  <button style={styles.checkCircle} onClick={() => onCompleteTask(task.id)} title="Mark complete" />
                </td>
                <td style={{ ...styles.td, fontWeight: 500, cursor: 'pointer' }} onClick={() => onStartFocus(task.id)}>{task.content}</td>
                <td style={styles.td}>
                ...
                {/* FIX-4: show outcome + motivation with tooltip for full text */}
                <td style={{ ...styles.td, maxWidth: 200 }}>
                  {outcome && (
                    <div style={{ fontSize: 12, color: '#52525B', marginBottom: 2 }} title={outcome}>
                      <span style={{ fontWeight: 600 }}>✓ </span>{outcome.length > 60 ? outcome.slice(0, 60) + '…' : outcome}
                    </div>
                  )}
                  {motivation && (
                    <div style={{ fontSize: 11, color: '#71717A' }} title={motivation}>
                      <span style={{ fontWeight: 600 }}>💡 </span>{motivation.length > 60 ? motivation.slice(0, 60) + '…' : motivation}
                    </div>
                  )}
                  {!outcome && !motivation && <span style={{ color: '#D4D4D8', fontSize: 13 }}>—</span>}
                </td>
  ...
  const PoolView = () => {
    ...
    const getRelLabel = (taskId) => poolRels.filter(r => r.fromTaskId === taskId || r.toTaskId === taskId);
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: '#71717A' }}>Pool:</span>
          <select value={selectedPoolId || ''} onChange={e => setSelectedPoolId(e.target.value)} style={{ ...styles.input, flex: 1, minWidth: 140 }}>
        ...
        {pool && (
          <div>
            <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
              {/* FIX-7: use text ⊕ instead of SVG icon component for reliability */}
                ...
                {poolTasks.map(task => {
                  ...
                  return (
                    <div key={task.id} style={{ ...styles.taskCard, borderLeft: '3px solid #6366F1' }}>
                      <div style={styles.taskCardHeader}>
                        <button style={styles.checkCircle} onClick={() => onCompleteTask(task.id)} />
                        <span style={{ ...styles.taskCardTitle, cursor: 'pointer' }} onClick={() => onStartFocus(task.id)}>{task.content}</span>
                      </div>
                      {/* FIX-4: show reflection in pool view */}
                      <ReflectionSummary reflection={task.reflection} />
                      {rels.length > 0 && (
  ...
  const PodView = () => {
      ...
      return (
        <div>
          ...
          {sorted.map(task => {
            ...
            return (
              <div key={task.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 10, border: `1px solid ${color}30`, backgroundColor: `${color}08`, marginBottom: 6, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 120 }}>
                  ...
                  {/* FIX-4: show reflection in pod annual view */}
                  {task.reflection?.motivation && (
                    <div style={{ fontSize: 11, color: '#71717A', marginTop: 2 }}>💡 {task.reflection.motivation.slice(0, 80)}{task.reflection.motivation.length > 80 ? '…' : ''}</div>
                  )}
                ...
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, border: `1px solid ${marked ? '#10B981' : color}`, backgroundColor: marked ? '#DCFCE7' : 'white', color: marked ? '#16A34A' : color, fontSize: 12, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}
                >
    ...
    // FIX-9: RecurringView — prevent marking future dates complete
    const RecurringView = () => {
      ...
      // FIX-9: only allow cycling for past/today, not future
      ...
      return (
        <>
          ...
          <div style={{ overflowX: 'auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: `160px repeat(14, 38px)`, gap: 3, marginBottom: 4, minWidth: 700 }}>
              ...
              {days.map(d => {
                ...
                return (
                  <div key={d} style={{ textAlign: 'center', fontSize: 10, color: isT ? '#FF6B6B' : (isFuture ? '#D4D4D8' : (active ? '#18181B' : '#D4D4D8')), fontWeight: isT ? 700 : 400, lineHeight: 1.3 }}>
                    <div>{WEEK_DAYS_SHORT[(new Date(d).getDay() + 6) % 7]}</div>
                    <div>{new Date(d).getDate()}</div>
                    {isFuture && <div style={{ fontSize: 8, color: '#D4D4D8' }}>—</div>}
                  </div>
                );
              })}
            </div>
            ...
            {podTasks.map(task => (
              <div key={task.id}>
                {/* FIX-4: show task reflection note */}
                {task.reflection?.motivation && (
                  <div style={{ fontSize: 11, color: '#71717A', paddingLeft: 4, paddingBottom: 2, fontStyle: 'italic' }}>💡 {task.reflection.motivation.slice(0, 80)}</div>
                )}
                <div style={{ display: 'grid', gridTemplateColumns: `160px repeat(14, 38px)`, gap: 3, marginBottom: 2, alignItems: 'center', minWidth: 700 }}>
                  ...
                  {days.map(d => {
                    ...
                    const canEdit = d >= addDays(today, -5) && !isFuture; // FIX-9: no future
                    return (
                      <div key={d}
                        style={{ textAlign: 'center', cursor: canEdit ? 'pointer' : 'default', opacity: isFuture ? 0.3 : 1 }}
                        title={isFuture ? 'Cannot mark future dates' : (canEdit ? 'Click to cycle: planned → done → missed' : 'Locked')}
                        onClick={() => canEdit && setPodLog(pod.id, task.id, d, { status: cycleStatus(log.status) })}
                      >
                        <span style={{ fontSize: 14 }}>{isFuture ? '·' : statusIcon(log.status)}</span>
                      </div>
                    );
                  })}
                </div>
                {(pod.trackerFields || []).filter(f => f.name).map(field => (
                  <div key={field.id} style={{ display: 'grid', gridTemplateColumns: `160px repeat(14, 38px)`, gap: 3, marginBottom: 2, alignItems: 'center', minWidth: 700 }}>
                    ...
                    {days.map(d => {
                      ...
                      const canEdit = d >= addDays(today, -5) && !isFuture; // FIX-9
                      if (field.type === 'checkbox') return (
                        <div key={d} style={{ textAlign: 'center', opacity: isFuture ? 0.3 : 1 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 11, color: '#A1A1AA' }}>· = not scheduled · click to cycle · locked after 5 days · future dates locked</span>
          </div>
        </>
      );
    };
    ...
    return (
      <div style={{ display: 'flex', gap: 16, overflow: 'hidden', flexWrap: 'wrap' }}>
            ...
            <div key={p.id} onClick={() => setSelectedPodId(p.id)} style={{ ...styles.podSideItem, ...(p.id === selectedPodId ? { backgroundColor: '#0EA5E915', borderColor: '#0EA5E9' } : {}) }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <span style={{ color: '#0EA5E9', flexShrink: 0, fontSize: 14 }}>↻</span>
        ...
        <div style={{ flex: 1, overflowX: 'auto', minWidth: 0 }}>
          {!pod && <EmptyState icon={<span style={{ fontSize: 32 }}>↻</span>} message="Select a Pod to see its schedule" />}
          {pod && (
            <>
  ...
  // FIX-10: EnergyView — new headings with number labels, renamed "Daily Plan"
  const EnergyView = () => {
    ...
    // FIX-10: heading labels updated with (1), (3), (5) notation
    const levels = {
      high: { title: 'High Energy Required (1)', emoji: '🔥', color: '#EF4444', description: 'Deep work — schedule first', tasks: [] },
      medium: { title: 'Medium Energy (3)', emoji: '⚡', color: '#F59E0B', description: 'Regular focus tasks', tasks: [] },
      low: { title: 'Low Energy (5)', emoji: '🌊', color: '#10B981', description: 'Easy wins — do anytime', tasks: [] },
    };
          ...
          return (
            <div key={key} style={{ ...styles.energySection, borderLeftColor: lv.color }}>
              <div style={styles.energyHeader}>
                ...
                <div>
                  <div style={styles.energyTitle}>{lv.title}</div>
                  <div style={styles.energyDesc}>{lv.description}</div>
                </div>
    ...
    <div>
      ...
      <div style={styles.workHeaderWithSelector}>
        <div style={styles.workTitleRow}>
          ...
          <select value={workView} onChange={e => setWorkView(e.target.value)} style={styles.viewSelector}>
            ...
            {/* FIX-10: renamed "Energy" → "Daily Plan" */}
            <option value="energy">Daily Plan</option>
        ...
        <p style={styles.workSubtitle}>
          ...
          {workView === 'energy' && 'Your daily plan — tasks grouped by energy level'}
      ...
      {pendingTasks.length === 0 && completedTasks.length === 0 && workView !== 'pool' && workView !== 'pod' ? (
        ...
        <>
          ...
          {/* FIX-2: Completed tasks shown in work mode with click to un-complete */}
          {workView !== 'pool' && workView !== 'pod' && completedTasks.length > 0 && (
            <div style={styles.completedSection}>
              <p style={styles.completedLabel}>Completed — click to restore</p>
              {completedTasks.map(t => (
                <div
                  key={t.id}
                  onClick={() => onUncompleteTask(t.id)}
                  style={{ ...styles.completedTask, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
                  title="Click to mark as not complete"
                >
                  <span style={{ color: '#10B981', fontSize: 12 }}>✓</span>
                  {t.content}
                  <span style={{ fontSize: 11, color: '#A1A1AA', marginLeft: 'auto' }}>undo</span>
                </div>
              ))}
            </div>
          )}
...
// ============================================================================
// REVIEW MODE — FIX-2: clicking task in review brings it back to work
// ============================================================================
function ReviewMode({ stats, reviews, tasks, onUncompleteTask }) {
  return (
    <div>
      ...
      {/* FIX-2: Show completed tasks in review, click to bring back to work */}
      {tasks.filter(t => t.isCompleted).length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <h4 style={{ ...styles.reviewListTitle, marginBottom: 8 }}>Completed Tasks — click to restore to Work</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {tasks.filter(t => t.isCompleted).slice(0, 8).map(t => (
              <div
                key={t.id}
                onClick={() => onUncompleteTask(t.id)}
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 8, backgroundColor: '#F9FAFB', border: '1px solid #E4E4E7', cursor: 'pointer', transition: 'background .15s' }}
                onMouseEnter={e => e.currentTarget.style.backgroundColor = '#F0FDF4'}
                onMouseLeave={e => e.currentTarget.style.backgroundColor = '#F9FAFB'}
              >
                <span style={{ color: '#10B981', fontSize: 14 }}>✓</span>
                <span style={{ flex: 1, fontSize: 13, color: '#18181B', textDecoration: 'line-through', opacity: 0.7 }}>{t.content}</span>
                <span style={{ fontSize: 11, color: '#4299E1', fontWeight: 600 }}>↩ Restore</span>
              </div>
            ))}
          </div>
        </div>
...
// ============================================================================
// STYLES — FIX-5: mobile optimization added
// ============================================================================
const styles = {
  ...
  headerContent: { maxWidth: '1200px', margin: '0 auto', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' },
  ...
  sup: { fontSize: '14px' },
  // FIX-5: Mobile — nav hides labels on small screens via inline styles
  headerNav: { display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' },
  headerRight: { display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 },
  greeting: { color: '#71717A', fontSize: '14px' },
  primaryBtn: { display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 14px', backgroundColor: '#FF6B6B', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', flexShrink: 0 },
  outlineBtn: { padding: '8px 14px', backgroundColor: 'transparent', color: '#52525B', border: '1px solid #D4D4D8', borderRadius: '8px', fontSize: '14px', cursor: 'pointer', flexShrink: 0 },
  smallOutlineBtn: { display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '5px 10px', backgroundColor: 'transparent', color: '#52525B', border: '1px solid #D4D4D8', borderRadius: '6px', fontSize: '13px', cursor: 'pointer', flexShrink: 0 },
  ...
  iconBtn: { width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#FF6B6B', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', flexShrink: 0 },
  ...
  formatBtn: { width: '28px', height: '28px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'white', color: '#52525B', border: '1px solid #D4D4D8', borderRadius: 5, cursor: 'pointer', flexShrink: 0 },
  main: { maxWidth: '1200px', margin: '0 auto', padding: '0 16px' },
  hero: { textAlign: 'center', padding: '40px 16px 28px' },
  heroContent: { maxWidth: '600px', margin: '0 auto' },
  heroTitle: { fontSize: 'clamp(24px, 5vw, 42px)', fontWeight: '800', color: '#18181B', lineHeight: 1.2, marginBottom: '12px' },
  tWord: { color: '#FF6B6B', display: 'inline-block', minWidth: '120px' },
  heroSubtitle: { fontSize: '15px', color: '#71717A', lineHeight: 1.6 },
  guestSection: { paddingBottom: '40px' },
  guestCard: { backgroundColor: 'white', border: '1px solid #E4E4E7', borderRadius: '16px', padding: '20px 16px', maxWidth: '800px', margin: '0 auto', boxShadow: '0 4px 16px rgba(0,0,0,0.04)' },
  // FIX-5: mode nav wraps on mobile
  modeNav: { display: 'flex', justifyContent: 'center', gap: '4px', backgroundColor: '#F4F4F5', padding: '4px', borderRadius: '100px', marginBottom: '20px', flexWrap: 'wrap' },
  modeBtn: { display: 'flex', alignItems: 'center', gap: '4px', padding: '7px 10px', backgroundColor: 'transparent', color: '#71717A', border: 'none', borderRadius: '100px', fontSize: '13px', fontWeight: '500', cursor: 'pointer', transition: 'all 0.2s' },
  modeBtnActive: { backgroundColor: '#FF6B6B', color: 'white' },
  modeBtnDisabled: { opacity: 0.5, cursor: 'not-allowed' },
  // FIX-5: input row wraps properly
  inputRow: { display: 'flex', gap: '8px', marginBottom: '12px', alignItems: 'flex-start' },
  input: { flex: 1, padding: '10px 14px', border: '1px solid #D4D4D8', borderRadius: '8px', fontSize: '14px', outline: 'none', backgroundColor: 'white', boxSizing: 'border-box' },
  // FIX-6: removed forced minHeight so Freedom mode is compact on first run
  taskArea: {},
  taskList: { display: 'flex', flexDirection: 'column', gap: '4px' },
  taskItem: { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '10px 12px', borderRadius: '8px', backgroundColor: '#F9FAFB', gap: '10px' },
  taskContent: { flex: 1, fontSize: '14px', color: '#18181B', lineHeight: 1.5 },
  taskActions: { display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 },
  typeBadge: { fontSize: 11, padding: '2px 6px', borderRadius: 4, fontWeight: 600, whiteSpace: 'nowrap' },
  // FIX-2: round checkbox circle
  checkCircle: { width: 18, height: 18, borderRadius: '50%', border: '2px solid #D4D4D8', backgroundColor: 'transparent', cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-color .15s' },
  completedSection: { marginTop: '20px', paddingTop: '12px', borderTop: '1px dashed #E4E4E7' },
  completedLabel: { fontSize: '13px', fontWeight: '600', color: '#71717A', marginBottom: '6px' },
  ...
  emptyIcon: { color: '#D4D4D8', marginBottom: '10px', display: 'flex', justifyContent: 'center' },
  emptyMessage: { color: '#71717A', fontSize: '14px', marginBottom: '14px' },
  // Focus
  focusMode: { padding: '16px 0' },
  progressDots: { display: 'flex', justifyContent: 'center', gap: '6px', marginBottom: '20px', flexWrap: 'wrap' },
  dot: { width: '8px', height: '8px', borderRadius: '100px', backgroundColor: '#E4E4E7', transition: 'all 0.3s', flexShrink: 0 },
  dotActive: { width: '32px', backgroundColor: '#FF6B6B' },
  focusHeader: { textAlign: 'center', marginBottom: '20px' },
  focusLabel: { fontSize: '13px', color: '#71717A', marginBottom: '6px' },
  focusTask: { fontSize: '18px', fontWeight: '600', color: '#18181B' },
  wizardCard: { backgroundColor: '#F9FAFB', borderRadius: '12px', padding: '20px 16px', maxWidth: '460px', margin: '0 auto' },
  questionHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', gap: 6 },
  wizardProgress: { fontSize: '13px', color: '#71717A' },
  wizardQuestion: { fontSize: '16px', fontWeight: '600', color: '#18181B', marginBottom: '14px' },
  purposeBadge: { fontSize: '11px', padding: '3px 8px', borderRadius: '100px', fontWeight: '600' },
  textarea: { width: '100%', minHeight: '90px', padding: '12px', border: '1px solid #D4D4D8', borderRadius: '8px', fontSize: '14px', resize: 'none', boxSizing: 'border-box', fontFamily: 'inherit' },
  optionalHint: { fontSize: '12px', color: '#A1A1AA', marginTop: '6px' },
  wizardActions: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '14px', flexWrap: 'wrap', gap: 8 },
  wizardRightActions: { display: 'flex', gap: '8px', alignItems: 'center' },
  doneIcon: { width: '48px', height: '48px', backgroundColor: '#DCFCE7', color: '#16A34A', borderRadius: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' },
  doneTitle: { fontSize: '18px', fontWeight: '700', color: '#18181B', textAlign: 'center', marginBottom: '6px' },
  doneText: { color: '#71717A', fontSize: '14px', textAlign: 'center', marginBottom: '16px' },
  skipSection: { textAlign: 'center', marginTop: '14px' },
  linkBtn: { background: 'none', border: 'none', color: '#A1A1AA', fontSize: '13px', cursor: 'pointer', textDecoration: 'underline' },
  loadingCard: { textAlign: 'center', padding: '28px', color: '#A1A1AA' },
  ...
  dayBtn: { width: 36, height: 36, border: '1px solid #D4D4D8', borderRadius: 6, fontSize: 12, cursor: 'pointer', backgroundColor: 'white', fontWeight: 500 },
  ...
  viewSelector: { padding: '6px 10px', border: '1px solid #D4D4D8', borderRadius: '8px', fontSize: '13px', fontWeight: '600', backgroundColor: 'white', cursor: 'pointer', color: '#18181B' },
  tableContainer: { overflowX: 'auto', WebkitOverflowScrolling: 'touch' },
  table: { width: '100%', borderCollapse: 'collapse', minWidth: 500 },
  th: { textAlign: 'left', padding: '8px 10px', fontSize: '11px', fontWeight: '600', color: '#71717A', borderBottom: '1px solid #E4E4E7' },
  tr: { borderBottom: '1px solid #F4F4F5' },
  td: { padding: '8px 10px', fontSize: '13px', color: '#18181B' },
  tdMuted: { padding: '8px 10px', fontSize: '12px', color: '#71717A', maxWidth: '120px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  timerCell: { display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px', flexWrap: 'wrap' },
  timerDisplay: { fontSize: '13px', color: '#71717A', fontVariantNumeric: 'tabular-nums', minWidth: '40px' },
  kanbanContainer: { display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '8px', WebkitOverflowScrolling: 'touch' },
  kanbanLane: { flex: '0 0 180px', display: 'flex', flexDirection: 'column' },
  ...
  kanbanLaneTitle: { fontSize: '12px', fontWeight: '600', color: '#52525B' },
  ...
  kanbanEmpty: { fontSize: '12px', color: '#D4D4D8', textAlign: 'center', padding: '14px 0' },
  taskCard: { backgroundColor: 'white', border: '1px solid #E4E4E7', borderRadius: '8px', padding: '10px 12px' },
  taskCardHeader: { display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '4px' },
  taskCardTitle: { fontSize: '13px', fontWeight: '500', color: '#18181B', flex: 1, lineHeight: 1.4 },
  taskCardMeta: { display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#A1A1AA', marginBottom: '4px', marginTop: 4 },
  taskCardActions: { display: 'flex', alignItems: 'center', gap: '4px', marginTop: '6px', flexWrap: 'wrap' },
  ...
  energyContainer: { display: 'flex', flexDirection: 'column', gap: '14px' },
  energySection: { borderLeft: '4px solid #ccc', paddingLeft: '12px' },
  energyHeader: { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px', flexWrap: 'wrap' },
  energyEmoji: { fontSize: '20px', flexShrink: 0 },
  ...
  statCard: { backgroundColor: '#F9FAFB', borderRadius: '12px', padding: '16px', textAlign: 'center' },
  statValue: { fontSize: '26px', fontWeight: '800', color: '#18181B', marginBottom: '4px' },
  statLabel: { fontSize: '12px', color: '#71717A', fontWeight: '500' },
  reviewModal: { backgroundColor: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: '12px', padding: '16px', marginBottom: '16px' },
  reviewTitle: { fontSize: '15px', fontWeight: '700', color: '#18181B', marginBottom: '4px' },
  reviewSubtitle: { fontSize: '13px', color: '#71717A', marginBottom: '14px' },
  ratingButtons: { display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' },
  ...
  reviewActions: { display: 'flex', gap: '8px', flexWrap: 'wrap' },
  reviewListTitle: { fontSize: '14px', fontWeight: '600', color: '#18181B', marginBottom: '10px' },
  reviewList: { display: 'flex', flexDirection: 'column', gap: '8px' },
  reviewItem: { backgroundColor: '#F9FAFB', borderRadius: '8px', padding: '12px' },
  reviewItemHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px', flexWrap: 'wrap', gap: 6 },
  ...
  featuresTitle: { fontSize: '22px', fontWeight: '700', color: '#18181B', textAlign: 'center', marginBottom: '20px' },
  featuresGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px' },
  featureCard: { padding: '18px', backgroundColor: 'white', borderRadius: '12px', border: '1px solid #E4E4E7' },
  featureIcon: { width: '40px', height: '40px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '10px' },
  ...
  ctaSection: { textAlign: 'center', marginTop: '28px', paddingTop: '20px', borderTop: '1px solid #F4F4F5' },
  ...
  authCard: { width: '100%', backgroundColor: 'white', borderRadius: '16px', padding: '28px 24px', border: '1px solid #E4E4E7', boxShadow: '0 4px 16px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: '14px' },
  authHeader: { textAlign: 'center', marginBottom: '4px' },
  ...
  answerSummary: { backgroundColor: 'white', borderRadius: '8px', padding: '12px', marginBottom: '14px', border: '1px solid #E4E4E7' },
  summaryTitle: { fontSize: '12px', fontWeight: '600', color: '#71717A', marginBottom: '6px' },
